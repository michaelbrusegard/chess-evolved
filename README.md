...
6. Add a env (no dot) file to the `/assets` folder and add `SUPABASE_URL={project_url}` and `SUPABASE_ANON_KEY={anon_key}` to this file.
7. While your database is running, you can access it's dashboard at <http://localhost:54323>. 
8. Go to this dashboard, and click the button at the bottom left of the webpage called "Command Menu".
9. Type in `copy api url`, press enter, and replace `{project_url}` in your .env and env file with whatever gets copied to your clipboard.
10. Type in `get api keys`, press enter, type `copy anonymous api key`, press enter and replace `{anon_key}` in your .env file with whatever gets copied to your clipboard.
11. You can now run the project, and it should connect to your local database.
12. When you're done developing, you can run `supabase stop` to stop your database.
